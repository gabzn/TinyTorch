...
def tensor_map(fn: Callable[[float], float]) -> Any:
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        out_idx: Index = np.zeros(MAX_DIMS, np.int16)
        in_idx: Index = np.zeros(MAX_DIMS, np.int16)

        # Fill in each position in out_storage
        for ordinal_idx in range(len(out_storage)):
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        for ordinal_idx in range(len(out_storage)):
            ...
            for reduced_idx in range(reduced_range):
                out_idx[reduce_dim] = reduced_idx
                j = index_to_position(out_idx, a_strides)
                out_storage[ordinal_out_idx] = fn(out_storage[ordinal_out_idx], a_storage[j])
